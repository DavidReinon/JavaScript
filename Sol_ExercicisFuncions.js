...
dies.forEach((unDia) => {
    numCaracters.push(unDia.length);
});
...
dies.forEach((unDia, index) => {
    console.log(unDia + ": " + numCaracters[index]);
});
...
let _mostraDiesMesDe = (unDia, numCaracters, acc) => {
    if (unDia.length > numCaracters) return acc + unDia + ", ";
    else return acc + "";
};
let stringAmbDiesMesDe7 = dies.reduce(
    (acc, unDia) => _mostraDiesMesDe(unDia, 7, acc),
    ""
);
console.log(stringAmbDiesMesDe7);
let rdo = dies.reduce(
    (acc, unDia) => (unDia.length > 7 ? acc + unDia + ", " : acc),
    ""
);
console.log(rdo);
